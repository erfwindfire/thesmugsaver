        ...
        <div className="not-prose my-10 rounded-lg overflow-hidden border-l-4 border-[#1B4D3E] bg-[#EBF2F1] shadow-sm max-w-[720px] mx-auto">
            <div className="p-8">
                <h3 className="font-serif font-bold text-xl text-[#1B4D3E] mb-4 flex items-center gap-3">
                    <span className="text-sm font-bold uppercase tracking-widest text-[#1B4D3E]/70 border-b-2 border-[#1B4D3E]/20 pb-1">Key Points</span>
                </h3>
                ...
                {actions && actions.length > 0 && (
                    <ul className="mt-4 pt-4 border-t border-[#1B4D3E]/20 space-y-2 list-none">
                        {actions.map((action, index) => (
                            <li key={index} className="flex items-start gap-2 text-sm text-gray-700">
                                <span className="mt-0.5 min-w-[16px] text-[#1B4D3E] font-bold text-base leading-tight">✓</span>
                                <span>{action}</span>
                            </li>
                        ))}
                    </ul>
